...
int main(void)
{
  ...
  /* hang mode test
    ...
    * the timxclk is fixed to 200 mhz, the prescaler is equal to 3 so the tmrx clock
      counter is equal to 50 mhz.
      the three timers are running at:
      tmrx frequency = tmrx clock counter/ 2*(timx_period + 1) = 500 khz.
  */

  /* tmr2 and tmr3 counter mode configuration */
  tmr_base_init(TMR1, 49, 3);
  ...
  tmr_channel_value_set(TMR1, TMR_SELECT_CHANNEL_1, 25);
